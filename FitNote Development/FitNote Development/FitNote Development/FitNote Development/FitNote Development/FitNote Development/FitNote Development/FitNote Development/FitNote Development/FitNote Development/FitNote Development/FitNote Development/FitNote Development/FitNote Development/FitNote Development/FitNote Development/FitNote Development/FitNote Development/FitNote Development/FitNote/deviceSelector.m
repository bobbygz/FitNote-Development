...
{
    //BG Call the parent clas init first to create the TVC
    self = [super initWithStyle:style];
    
    
    
    //BG Now create the objects unique to the deviceSelector class we defined. Nothing new is displayed except the title
    if (self) {
        // Custom initialization
        self.m = [[CBCentralManager alloc]initWithDelegate:self queue:nil];
        self.nDevices = [[NSMutableArray alloc]init];
...
- (void) prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender{
    if ([[segue identifier] isEqualToString:@"StrengthActivity"]){
