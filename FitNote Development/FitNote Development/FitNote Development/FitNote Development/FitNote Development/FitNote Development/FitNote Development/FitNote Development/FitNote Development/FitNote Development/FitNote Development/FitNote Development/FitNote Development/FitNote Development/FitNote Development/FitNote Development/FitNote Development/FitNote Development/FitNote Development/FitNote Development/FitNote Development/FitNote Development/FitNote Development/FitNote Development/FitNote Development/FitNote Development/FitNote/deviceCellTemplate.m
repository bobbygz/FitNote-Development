...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
        ...
        self.deviceName.font = [UIFont boldSystemFontOfSize:20];
...
-(void)layoutSubviews {
    ...
    fr = CGRectMake(boundsX + 10, 2, 120, 120);
    self.deviceIcon.frame = fr;
    
    fr = CGRectMake(boundsX + 135, 50, self.contentView.bounds.size.width - 100, 25);
...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
       ...
       self.height = 250;
        ...
        //BG
        self.repcountLabel = [[UILabel alloc] init];
        self.repcountLabel.textAlignment = NSTextAlignmentLeft;
        self.repcountLabel.font = [UIFont boldSystemFontOfSize:17];
        self.repcountLabel.backgroundColor = [UIColor clearColor];
    ...
    //BG
        [self.contentView addSubview:self.repcountLabel];
...
-(void)layoutSubviews {
    ...
    if (contentRect.size.width < WIDTH_CHECKER) {
        ...
        self.repcountLabel.frame = fr;
        //BG self.accCalibrateButton.frame = fr;

    }
    
    //BG Landscape Orientation
    else {
...
@implementation fitnoteCellTemplate

@synthesize fitnLabel,fitnIcon,fitnState,fitnOrientation,fitnCalibrateButton,fitnrepcountLabel,height;
...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
        ...
        // Initialization code
        self.fitnLabel = [[UILabel alloc] init];
        self.fitnLabel.textAlignment = NSTextAlignmentLeft;
        self.fitnLabel.font = [UIFont boldSystemFontOfSize:34];
        self.fitnLabel.backgroundColor = [UIColor clearColor];
        
        
        self.fitnState = [[UILabel alloc] init];
        self.fitnState.textAlignment = NSTextAlignmentLeft;
        self.fitnState.font = [UIFont boldSystemFontOfSize:17];
        self.fitnState.backgroundColor = [UIColor clearColor];

        self.fitnOrientation = [[UILabel alloc] init];
        self.fitnOrientation.textAlignment = NSTextAlignmentLeft;
        self.fitnOrientation.font = [UIFont boldSystemFontOfSize:17];
        self.fitnOrientation.backgroundColor = [UIColor clearColor];

        self.fitnrepcountLabel = [[UILabel alloc] init];
        self.fitnrepcountLabel.textAlignment = NSTextAlignmentLeft;
        self.fitnrepcountLabel.font = [UIFont boldSystemFontOfSize:34];
        self.fitnrepcountLabel.backgroundColor = [UIColor clearColor];

        
        self.fitnIcon = [[UIImageView alloc] init];
        self.fitnIcon.image = [UIImage imageNamed:@"_FitScribeAppImage.png"];
        [self.fitnIcon setAutoresizingMask:UIViewAutoresizingNone];
        self.fitnIcon.contentMode = UIViewContentModeScaleAspectFit;
        
        
        self.fitnCalibrateButton = [UIButton buttonWithType:UIButtonTypeRoundedRect];
        //[self.fitnCalibrateButton setTitle:@"Calibrate and Reset" forState:UIControlStateNormal];
        
        //BG
        [self.contentView addSubview:self.fitnLabel];
        [self.contentView addSubview:self.fitnIcon];
        [self.contentView addSubview:self.fitnCalibrateButton];
        [self.contentView addSubview:self.fitnState];
        [self.contentView addSubview:self.fitnOrientation];
        [self.contentView addSubview:self.fitnrepcountLabel];
    }
    return self;
}
...
-(void)layoutSubviews {
    ...
    //BG Check to be sure in portrait mode
    if (self.contentView.bounds.size.width < WIDTH_CHECKER) self.height = 220;
    else self.height = 150;
    
    if (contentRect.size.width < WIDTH_CHECKER) {
        //icon centered in cell vertically, ends at x = 75
        // 100 pts high
        
        fr = CGRectMake(contentRect.origin.x + 5, 75, 120, 120);
        self.fitnIcon.frame = fr;
        
        // BG Cell Label 5pts down, centered horizontally
        // 25 high, ends at y=30
        fr = CGRectMake(contentRect.origin.x, 5, self.contentView.bounds.size.width, 50);
        self.fitnLabel.textAlignment = NSTextAlignmentCenter;
        self.fitnLabel.frame = fr;
        
        
        fr = CGRectMake(contentRect.origin.x + self.contentView.bounds.size.width - 25, 61, -150, 50);

        self.fitnState.frame = fr;
        fr.origin.y += 35;
        self.fitnOrientation.frame = fr;
        fr.origin.y += 35;
        self.fitnrepcountLabel.frame = fr;
        
   //     fr = CGRectMake((contentRect.origin.x + (contentRect.size.width / 2 ) - 75), 85,95,50);
  //      self.accGraphHolder.frame = fr;
        
        fr = CGRectMake(contentRect.origin.x + (self.contentView.bounds.size.width / 2) - 65,180,150,35);
        self.fitnCalibrateButton.frame = fr;
